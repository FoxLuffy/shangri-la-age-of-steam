...
async def simulate_faction_wars():
    from backend.database import Faction, FactionStanding, WorldState
...
dummy_state = WorldState(current_location_id="1", active_npcs_ids=[], world_memories=[])
...
async def craft_item(character_id: int, recipe_id: int):
    ...
    """
    from backend.database import Inventory, WorldState
...
async def trade_market(character_id: int, req: MarketTradeRequest):
    from backend.database import Character, Inventory, Item
