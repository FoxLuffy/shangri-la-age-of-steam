...
class StateRepository:
    ...
    def get_latest_state(self) -> Optional[WorldState]:
        ...
        return self.session.exec(select(WorldState).order_by(WorldState.id.desc())).first()
    ...
    def update_location(self, location_id: str, data: Dict[str, Any]) -> Optional[Location]:
        location = self.session.exec(select(Location).where(Location.id == location_id)).first()
        if location:
