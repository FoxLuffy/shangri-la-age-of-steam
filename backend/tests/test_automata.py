...
def test_create_and_fetch_automata():
    ...
    with Session(db_engine) as session:
        ...
        owl = AutomataCompanion(
            name="Clockwork Owl", model_type="scout", core_power=100.0, modules=["optical_lens", "brass_wings"]
        )
        session.add(owl)
        session.commit()
        session.refresh(owl)
...
def test_api_get_automata():
    ...
    with Session(db_engine) as session:
        hound = AutomataCompanion(
            name="Brass Hound", model_type="combat", core_power=150.0, modules=["pneumatic_jaw", "reinforced_chassis"]
        )
        session.add(hound)
        session.commit()
...
def test_worldstate_can_have_active_automata():
    from backend.database import AutomataCompanion, WorldState

    with Session(db_engine) as session:
        owl = AutomataCompanion(name="Clockwork Owl", model_type="scout", core_power=100.0, modules=[])
        session.add(owl)
        session.commit()
        session.refresh(owl)
