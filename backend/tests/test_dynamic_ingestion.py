import pytest
from backend.database import NPC, Item, Location
...
[StateUpdates]
{
    ...
    action = PlayerAction(action_text="look around", current_location_id="1", character_id=1)

    # Process the action
    list(engine.process_action(action, session=engine_session))
