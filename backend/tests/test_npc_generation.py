...
def test_procedural_npc_generation_logic():
    ...
    with Session(db_engine) as session:
        ...
        role = npc.traits[0]
        assert (
            "industrial" in role.lower()
            or "steam" in role.lower()
            or "factory" in role.lower()
            or "brass" in role.lower()
            or "engineer" in role.lower()
            or "worker" in role.lower()
            or "machinist" in role.lower()
            or "foreman" in role.lower()
            or "shoveler" in role.lower()
        )
