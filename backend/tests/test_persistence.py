from backend.database import NPC, Location, WorldState
...
def test_persistence():
    ...
    with Session(test_engine) as session:
        loc = Location(id="loc_1", name="The Rusty Anchor", description="A bustling tavern in the dock district.")
        session.add(loc)
        session.commit()

        npc = NPC(id="npc_1", name="Barnaby", traits=["brave", "drunk"], location_id="loc_1")
        session.add(npc)
        session.commit()

        state = WorldState(current_location_id="loc_1", global_event="Festival of Sails")
        session.add(state)
        session.commit()
