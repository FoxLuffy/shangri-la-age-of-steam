...
export default function StatsPanel({ character, worldState, onReset, onOpenEmpire, onOpenSettings, onOpenMarket, onOpenWorkshop }: { character: Character, worldState?: any, onReset: () => void, onOpenEmpire?: () => void, onOpenSettings?: () => void, onOpenMarket?: () => void, onOpenWorkshop?: () => void }) {
  return (
    <div className="w-64 bg-slate-900 border-l border-amber-900/30 flex flex-col p-4 overflow-y-auto">
      ...
      <div className="mt-auto pt-4 border-t border-amber-900/30 space-y-2">
        ...
        <button
          ...
          className="w-full px-3 py-2 text-xs bg-slate-800 hover:bg-amber-900/40 text-amber-500 hover:text-amber-300 border border-slate-700 hover:border-amber-700/50 rounded transition-all flex items-center justify-center gap-2 uppercase tracking-wider"
          title="Steam Workshop Mods"
        >
          <span>🛠️</span> Mod Workshop
        </button>
        <button
          onClick={onReset}
